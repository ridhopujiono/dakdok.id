@extends('admin.layouts.main')
    ...
    <link rel="stylesheet" href="{{asset('admin/assets/vendors/toastify/toastify.css')}}">
        ...
        <div class="page-heading">
            <div class="m-auto">
                <a href="{{url('tambah_admin')}}" class="btn btn-primary rounded-pill shadow-sm"><span class=" bi bi-plus"></span> tambah admin</a>
            </div>
        </div>
                            ...
                            <div class="table-responsive px-5 pb-5">
                                <table class="table table-hover mb-0" id="table_iklan">
                                    ...
                                    <tbody id="tbody-iklan">
                                        ...
                                        <tr class="text-center">
                                            <td name="no" >{{ $no++ }}</td>
                                            <th name="nama_iklan" class="text-center">{{ $d->name }}</th>
                                            <td name="deskripsi_iklan">{{ $d->email }}</td>
                                            <td>
                                                <div class="d-inline">
                                                    <!-- <a href="{{url('edit_iklan')}}/{{$d->id_iklan}}" class="badge bg-primary">edit</a> -->
                                                    <form action="{{url('hapus_user_admin')}}/{{$d->id}}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('delete')
